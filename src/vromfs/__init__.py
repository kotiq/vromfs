...
class VromfsBinFile:
    ...
    def __init__(self, name_or_stream: t.Union[os.PathLike, io.BufferedReader]):
        ...
        self.container = BinContainer.parse_stream(stream)
    ...
    def print_dir(self, file=None):
        """Сводка об архиве в file"""

        files_info: t.Sequence[FileInfo] = self.container.info.files_info
        if not files_info:
            return
        ...
        for info in self.container.info.files_info:
            values = map(lambda g: g(info), getters)
            print(fmt.format(*values), file=file)

    def extract(self, item: FileItem, path: t.Optional[os.PathLike] = None) -> t.Sequence[Path]:
    ...
    def extract_all(self,
        ...
        return list(filter(lambda p: p is not None, map(lambda i: self._extract_item(i, path), items)))
